...
from new_config import BASE_URL_THAI, headless, th_close_all_popups_and_chat, thai, check_visible_text_for_errors
...
class TestChromiumBrowser(unittest.IsolatedAsyncioTestCase):
    ...
    async def asyncSetUp(self):
        ...
        await thai(self.page)
        await self.login()

    async def login(self):
        """执行登录操作"""
        await self.page.goto(BASE_URL_THAI)
        self.assertIn("WE88", await self.page.title())

        # 确保聊天窗口和弹窗已关闭
        await self.page.click("#langeDiv > a > img")
        await self.page.click("#region_language_modal > div > div > div > div > div.region-select-text > div:nth-child(3) > div.region-select-lang > a:nth-child(1)")
    ...
    async def test_th(self):
        await th_close_all_popups_and_chat(self.page)
        await self.page.click('button.ewallatbtn')
        await self.page.wait_for_timeout(1000)
        print('...Start Testing Thailand Crypto Deposit...')
        ...
        for data_id in data_ids:
            ...
            # 填写金额
            await self.page.fill(".fieldInput.mb-2 input[id='depAmount']", "100")
            await self.page.wait_for_timeout(1000)
        ...
        print("--------------------Thailand Crypto Testing Done--------------------\n")

        print('...Start Testing Thailand Fast Deposit...')
        await self.page.click('button.ewallatbtn')
        await self.page.wait_for_timeout(1000)
        ...
        for data_id in data_ids:
            ...
            await self.page.wait_for_selector(".input-wrap.bank-deposit.payment-gateway input[id='depAmount']",
                                              timeout=5000)
            await self.page.fill(".input-wrap.bank-deposit.payment-gateway input[id='depAmount']", "100")
        ...
        print("--------------------Thailand Fast Deposit Testing Done--------------------\n")
