...
class TestChromiumBrowser(unittest.IsolatedAsyncioTestCase):
    ...
    async def asyncSetUp(self):
        ...
        await viet(self.page)
        await self.login()

    async def login(self):
        """执行登录操作"""
        await self.page.goto(BASE_URL_VIET)
        self.assertIn("WE88", await self.page.title())

        # 确保聊天窗口和弹窗已关闭
        await self.page.click("#langeDiv > a > img")
        await self.page.click("#region_language_modal > div > div > div > div > div.region-select-text > div:nth-child(4) > div.region-select-lang > a:nth-child(1)")
    ...
    async def test_vn(self):
        await self.page.wait_for_timeout(1000)
        await vn_close_all_popups_and_chat(self.page)
        await self.page.click('button.ewallatbtn')
        await self.page.wait_for_timeout(1000)
        print('...Start Testing Vietnam Crypto Deposit...')
        ...
        for data_id in data_ids:
            ...
            # 填写金额
            await self.page.fill(".fieldInput.mb-2 input[id='depAmount']", "100000")
            await self.page.wait_for_timeout(1000)
        ...
        print("--------------------Vietnam Crypto Testing Done--------------------\n")

        print('...Start Testing Vietnam Fast Deposit...')
        ...
        print("--------------------Vietnam Fast Deposit Testing Done--------------------\n")
